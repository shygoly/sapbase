...
Required output shape (use exactly one of these formats):

Format A - states with embedded transitions:
{
  "states": [ { "name": "stateName", "initial": true|false, "transitions": ["targetStateName"] } ]
}
- Exactly one state must have "initial": true. States with no outgoing transitions are treated as final.

Format B - explicit transitions array:
{
  "states": [ { "name": "stateName", "initial": true|false, "final": true|false } ],
  "transitions": [ { "from": "stateName", "to": "targetStateName" } ]
}
