...
export class TransitionEngineService {
  ...
  ): { valid: boolean; error?: string; guardResult?: any } {
    // Find transition
  ...
  async executeTransition(
    ...
    // Validate transition
    const validation = this.validateTransition(workflow, instance, toState, entity)
    if (!validation.valid) {
    ...
    try {
      ...
      // Update entity state field if entityService is provided
      let updatedEntity = null
      if (entityService && entity && instance.entityId) {
        // Determine state field name (common patterns: 'state', 'status', 'workflowState')
        const stateField = this.determineStateFieldName(entity)
