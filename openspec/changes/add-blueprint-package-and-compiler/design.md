...
| 判 | 判据 |
| --- | --- |
| 重复定义 | 同一 `entity` / `flow` / `step` id 出现两次 |
| 悬空引用 | 引用了不存在的 entity / state / rule / atomic |
| 流程成环 | **flow 步骤图**存在环 —— **v1 规定 flow 是 DAG** |
| 状态机合法性 | 初始态唯一、存在终态、无不可达态、迁移目标已声明 |
| 原子依赖不可满足 | `AtomicRegistryService.resolve` 抛错 |

**不判**（会变成假阳性来源）：命名风格、语义相似度、"这个规则写得对不对"这类需要判断的检查。

### 一处自我修正：为什么**不**对实体关系图做环检测

初稿写的是"实体关系图或流程图的环（含自环）"。真去实现时发现这条判据**会大量误报**：

- `Employee.manager → Employee` 是合法的自引用；
- `Customer → Order → Customer`（如 `Order.billingCustomer` 回指）也是常见建模。

实体关系图有环不等于模型有问题。因此 v1 **只对 flow 步骤图做环检测**，并把
"flow 必须是 DAG"作为**协议约定**写下来（理由：v1 不做流程回环的语义分析，
需要回环的场景用**事件再次触发流程**表达，语义落在事件而不是图上的环）。
这样判据是确定性的，且不会误伤合法建模。
