...
export const BLUEPRINT_IR_VERSION = 'blueprint-ir/v1' as const

export type BlueprintDependency =
  | { atomic: string; version: string }
...
export interface BlueprintManifest {
  ...
  layers: Partial<Record<BlueprintLayerName, string[]>> & { public: string[] }
...
export type BlueprintIrActionKind =
  | 'check'
  | 'require-approval'
  | 'post-accounting'
...
export interface BlueprintIr {
  ...
  /** 编译时**实际解析到的**依赖版本 */
  dependencies: string[]
  summary?: { entities?: number; events?: number; files?: number }
}
