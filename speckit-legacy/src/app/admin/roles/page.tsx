...
export default function RolesPage() {
  ...
  const pageModel = useMemo(() => {
    return buildPageModel({
      ...
      path: '/admin/roles',
      fallbackTitle: 'Roles',
      fallbackDescription: 'Role management',
      pageSchema,
    })
  }, [pageSchema])
  ...
  const collectionSchema = useMemo(() => {
    return objectSchema
      ...
      : { id: 'roles-collection', title: 'Roles', columns: [] }
  }, [objectSchema, pageSchema])

  const formSchema = useMemo(() => {
    return objectSchema
      ? toFormSchema(objectSchema)
      : { id: 'role-form', title: 'Role', fields: [] }
  }, [objectSchema])
  ...
  return (
    <PageRuntime model={pageModel} className="p-6 space-y-6">
      ...
      {!showForm ? (
        <CollectionRuntime schema={collectionSchema} data={roles} isLoading={isLoading}>
