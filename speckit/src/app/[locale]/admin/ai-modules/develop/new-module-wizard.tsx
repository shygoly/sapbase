...
export function NewModuleDefinitionWizard({ onComplete, onCancel }: NewModuleDefinitionWizardProps) {
  ...
  const handleGenerateAll = useCallback(async () => {
    const userInput = getStepUserInput(0, state)
    if (!userInput.trim()) {
      setStepError('Enter a description in Step 1 (Object model) to generate all steps.')
      return
    }
    setGeneratingAll(true)
    setStepError(null)
    try {
      const merged = await aiModulesApi.generateAllDefinitionSteps(userInput.trim())
      setState((prev) => ({
        ...prev,
        objectModel: { ...(prev.objectModel || {}), generated: merged.objectModel },
        relationships: { ...(prev.relationships || {}), generated: merged.relationships },
        stateFlow: { ...(prev.stateFlow || {}), generated: merged.stateFlow },
        pages: { ...(prev.pages || {}), generated: merged.pages },
        permissions: { ...(prev.permissions || {}), generated: merged.permissions },
        reports: { ...(prev.reports || {}), generated: merged.reports },
      }))
    } catch (err: any) {
      setStepError(err?.response?.data?.message ?? err?.message ?? 'Generate all failed')
    } finally {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            disabled={generatingAll || !getStepUserInput(0, state).trim()}
          >
            {generatingAll ? 'Generating all…' : 'Generate all 6 steps'}
          </Button>
        </div>
