...
import { moduleRegistryApi } from '@/lib/api/module-registry.api'
...
export default function AIModuleTestPage() {
  ...
  useEffect(() => {
    if (selectedModuleId) {
      ...
      if (foundModule?.testResults?.tests) {
        // Convert test results to AIModuleTest format
        setTests(
          foundModule.testResults.tests.map((t: any) => ({
            id: t.id,
  ...
  const handleRunTests = async () => {
    ...
    let testModuleId = foundModule.aiModuleId
    if (!testModuleId && foundModule.registryId) {
      // For registry-only modules (like CRM), create a temporary AIModule for testing
      try {
        const tempModule = await aiModulesApi.create({
          name: foundModule.name,
          description: `Temporary module for testing ${foundModule.name}`,
        })
