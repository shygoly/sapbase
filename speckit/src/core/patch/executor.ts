...
export class PatchExecutor {
  ...
  async execute(
    ...
    try {
      ...
      if (!currentSchema) {
        currentSchema = this.loadSchema(patch) ?? undefined
        if (!currentSchema) {
          return {
            success: false,
  ...
  private applyPagePatch(
    ...
    switch (patch.operation) {
      case 'add':
        ...
        if (payload.metadata) {
          schema.metadata = {
            title: schema.metadata?.title ?? payload.metadata.title ?? '',
            ...schema.metadata,
            ...payload.metadata,
          }
        }
      ...
      case 'add':
        if (payload.field) {
          ...
          const newField: FieldDefinition = {
            ...
            validation: payload.field.validation as import('../schema/types').ValidationRule[] | undefined,
